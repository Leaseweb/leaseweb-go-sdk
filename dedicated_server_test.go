...
func TestDedicatedServerList(t *testing.T) {
	...
	assert.Equal(Server.PrivateNetworks[0].LinkSpeed, 1000)
...
func TestDedicatedServerGet(t *testing.T) {
	...
	assert.Equal(SoftwareLicense.Price, float32(12.12))
	assert.Equal(Server.Contract.BillingCycle, 12)
	...
	assert.Equal(PrivateNetwork.LinkSpeed, 1000)
	...
	assert.Equal(Server.Specs.Cpu.Quantity, 4)
...
func TestDedicatedServerCancelActiveJob(t *testing.T) {
	...
	assert.Equal(Task.ErrorMessage, "The job was canceled by the api consumer")
	...
	assert.Equal(Job.Progress.Canceled, 1)
	assert.Equal(Job.Progress.Expired, 0)
	assert.Equal(Job.Progress.Failed, 0)
	assert.Equal(Job.Progress.Finished, 0)
	assert.Equal(Job.Progress.InProgress, 0)
	assert.Equal(Job.Progress.Pending, 0)
	assert.Equal(Job.Progress.Percentage, 0)
	assert.Equal(Job.Progress.Total, 1)
	assert.Equal(Job.Progress.Waiting, 0)
	...
	assert.Equal(Job.Payload.OS.Architecture, "64bit")
	assert.Equal(Job.Payload.OS.Family, "ubuntu")
	assert.Equal(Job.Payload.OS.Name, "Ubuntu 20.04 LTS (Focal Fossa) (amd64)")
	assert.Equal(Job.Payload.OS.Type, "linux")
	assert.Equal(Job.Payload.OS.Version, "20.04")
	assert.Equal(Job.Payload.Partitions[0].FileSystem, "swap")
...
func TestDedicatedServerLunchHardwareScan(t *testing.T) {
	...
	assert.Equal(Task.StatusTimestamps.Pending, "2021-01-09T08:54:06+00:00")
	assert.Equal(Task.StatusTimestamps.Waiting, "2021-01-09T08:54:06+00:00")
	assert.Equal(Job.Progress.Canceled, 0)
	assert.Equal(Job.Progress.Expired, 0)
	assert.Equal(Job.Progress.Failed, 0)
	assert.Equal(Job.Progress.Finished, 0)
	assert.Equal(Job.Progress.InProgress, 0)
	assert.Equal(Job.Progress.Pending, 1)
	assert.Equal(Job.Progress.Percentage, 0)
	assert.Equal(Job.Progress.Total, 5)
	assert.Equal(Job.Progress.Waiting, 4)
	assert.Equal(Job.Payload.FileServerBaseUrl, "http://asd.asd")
...
func TestDedicatedServerLunchInstallation(t *testing.T) {
	...
	assert.Equal(Job.Progress.Pending, 1)
	assert.Equal(Job.Progress.Percentage, 0)
	assert.Equal(Job.Progress.Total, 26)
	assert.Equal(Job.Progress.Waiting, 25)
	...
	assert.Equal(Job.Payload.OS.Type, "linux")
	assert.Equal(Job.Payload.OS.Version, "20.04")
	assert.Equal(Job.Payload.Partitions[0].FileSystem, "ext2")
	assert.Equal(Job.Payload.Partitions[0].Size, "500")
	assert.Equal(Job.Payload.Partitions[0].Bootable, true)
	assert.Equal(Job.Payload.Partitions[0].MountPoint, "/boot")
	assert.Equal(Job.Payload.Partitions[0].Primary, true)
	assert.Equal(Job.Payload.Partitions[1].FileSystem, "swap")
	assert.Equal(Job.Payload.Partitions[1].Size, "4096")
	assert.Equal(Job.Payload.Partitions[2].FileSystem, "ext4")
	assert.Equal(Job.Payload.Partitions[2].MountPoint, "/tmp")
	assert.Equal(Job.Payload.Partitions[2].Size, "4096")
	assert.Equal(Job.Payload.Partitions[3].FileSystem, "ext4")
	assert.Equal(Job.Payload.Partitions[3].Size, "*")
	assert.Equal(Job.Payload.Partitions[3].MountPoint, "/")
...
func TestDedicatedServerGetJob(t *testing.T) {
	...
	assert.Empty(Job.Tasks[0].ErrorMessage)
...
func TestDedicatedServerLunchRescueMode(t *testing.T) {
	...
	assert.Equal(Job.Progress.Pending, 1)
	assert.Equal(Job.Progress.Percentage, 0)
	assert.Equal(Job.Progress.Total, 12)
	assert.Equal(Job.Progress.Waiting, 11)
...
func TestDedicatedServerGetBandWidthMetrics(t *testing.T) {
	...
	assert.Equal(Metric.Metric.DownPublic.Values[0].Value, 202499)
	assert.Equal(Metric.Metric.DownPublic.Values[0].Timestamp, "2016-10-20T09:00:00Z")
	assert.Equal(Metric.Metric.DownPublic.Values[1].Value, 29900)
	...
	assert.Equal(Metric.Metric.UpPublic.Values[0].Value, 43212393)
	assert.Equal(Metric.Metric.UpPublic.Values[0].Timestamp, "2016-10-20T09:00:00Z")
	assert.Equal(Metric.Metric.UpPublic.Values[1].Value, 12342929)
